...
impl Renderer {
    pub async fn new(source: impl SurfaceSource, width: u32, height: u32, x: u32, y: u32) -> Self {
        let mut ctx = GpuContext::init().await;
        ctx.add_surface(source, width, height, x, y).await;
        let ctx = ctx;
        let shader = ctx
            .device()
        ...
        let pipeline = ctx
            ...
            .create_render_pipeline(&wgpu::RenderPipelineDescriptor {
                ...
                fragment: Some(wgpu::FragmentState {
                    ...
                    targets: &[Some(ctx.surfaces()[0].current_format().into())],
                    compilation_options: Default::default(),
                }),
