...
class WasmSurface implements GpuSurface {
  ...
  render() { this.gpu.render() }
  dispose() {}

  static async create(canvas: HTMLCanvasElement): Promise<WasmSurface> {
    const wasmMod = await import('./wasm_gpu/wgpu_renderer.js')
    // Pass explicit WASM URL for Angular's esbuild to resolve the binary correctly
    await wasmMod.default(new URL('./wasm_gpu/wgpu_renderer_bg.wasm', import.meta.url))
...
export class WgpuPanel {
  ...
  async init(_params: GroupPanelPartInitParameters) {
    ...
    const loop = () => {
      ...
      if (r.width > 0 && r.height > 0) this.surface!.render()
